...
import {
  ...
  Plus, Pencil, Trash2, Loader2, Shield, Swords,
  Workflow,
} from "lucide-react";
...
import { BattlecardsSection } from "@/components/battlecards/BattlecardsSection";
...
function KnowledgeBase() {
  ...
  async function load() {
    ...
    try {
      const { data: pbData, error: pbError } = await supabase
        .from("interactive_playbooks" as any)
        .select("*") as any;
      if (pbError) throw pbError;
      if (pbData) setDbPlaybooks(pbData as any);
    } catch {
  ...
  async function handleDelete(id: string) {
    ...
    setDeleting(id);
    try {
      const { error } = await supabase.from("knowledge_flows" as any).delete().eq("id", id);
      if (error) throw error;
  ...
  async function handleDeletePlaybook(id: string) {
    ...
    try {
      const { error } = await supabase.from("interactive_playbooks" as any).delete().eq("id", id);
      if (error) throw error;
  ...
  return (
    <div className="flex flex-col gap-6 p-6 lg:p-8 max-w-[1400px] mx-auto pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-5 border-b border-border mb-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">Fluxos, playbooks, battlecards e manuais unificados</p>
          </div>
        </div>
      </div>
      ...
      <Tabs defaultValue="flows" className="w-full">
        <TabsList className="h-9 bg-secondary/30 border border-border rounded-lg p-1 mb-4 w-fit">
          <TabsTrigger
            value="flows"
            className="text-xs gap-1.5 data-[state=active]:bg-card data-[state=active]:text-foreground"
          >
            <BookOpen className="h-3.5 w-3.5" /> Fluxos & Playbooks
          </TabsTrigger>
          <TabsTrigger
            value="battlecards"
            className="text-xs gap-1.5 data-[state=active]:bg-card data-[state=active]:text-foreground"
          >
            <Swords className="h-3.5 w-3.5" /> Battlecards
          </TabsTrigger>
        </TabsList>

        {/* ── Tab: Fluxos & Playbooks ── */}
        <TabsContent value="flows" className="mt-0 flex flex-col gap-5">
