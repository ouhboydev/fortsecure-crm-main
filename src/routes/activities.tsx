...
export const Route = createFileRoute("/activities")({
  ...
  component: () => <AppShell><AgendaPage /></AppShell>,
});
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      ...
      const finalDesc = form.description
        ? `${isPublicStr}${form.description}`
        : form.is_public ? `[PÚBLICO] ${new Date().toISOString()}` : null;
  ...
  return (
    <div className="flex flex-col gap-6 p-6 lg:p-8 max-w-[1400px] mx-auto pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-border">
        <div>
          <h1 className="text-xl font-semibold text-foreground">Agenda Planejada</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Gestão de tarefas e compromissos da equipe</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Pendentes",  value: displayed.length,      color: "text-[#f59e0b]"    },
          { label: "Concluídas", value: completedItems.length, color: "text-[#3ecf8e]"    },
        ].map(s => (
      ...
      {view === "list" ? (
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              <AnimatePresence mode="popLayout">
                {displayed.map(a => (
                  <ActivityRow key={a.id} a={a} onComplete={complete} onRemove={remove} onEdit={openEdit} currentUserId={user?.id} />
                ))}
              </AnimatePresence>
              {displayed.length === 0 && (
        ...
        <CalendarView
          items={items.filter(i => i.status !== "concluida")}
...
// ─── ActivityRow ──────────────────────────────────────────────────
function ActivityRow({ a, onComplete, onRemove, onEdit, currentUserId }: {
  a: any; onComplete: any; onRemove: any; onEdit: any; currentUserId?: string
}) {
      ...
      <div className={cn(
        ...
        <div className="flex-1 min-w-0" onClick={() => isOwner && onEdit(a)} style={{ cursor: isOwner ? "pointer" : "default" }}>
          ...
          <div className="flex items-center gap-3 mt-0.5">
            ...
            {!isOwner && a.profiles && <span className="text-[10px] text-muted-foreground">por {a.profiles.full_name}</span>}
          </div>
        </div>

        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
          {isOwner && (
            <button onClick={() => onEdit(a)} className="h-7 w-7 rounded-md flex items-center justify-center text-muted-foreground hover:text-[#3ecf8e] hover:bg-[#3ecf8e]/10 transition-all">
              <Pencil className="h-3.5 w-3.5" />
            </button>
          )}
          {isOwner && (
            <button onClick={() => onRemove(a.id)} className="h-7 w-7 rounded-md flex items-center justify-center text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all">
...
// ─── CalendarView ─────────────────────────────────────────────────
function CalendarView({ items, selectedDate, onSelect, onNew, onEdit, user }: {
  items: any[]; selectedDate: Date; onSelect: any; onNew: any; onEdit: any; user: any
}) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="icon" onClick={prevMonth} className="h-8 w-8 border-border bg-card"><ChevronLeft className="h-3.5 w-3.5" /></Button>
          <Button variant="outline" size="icon" onClick={nextMonth} className="h-8 w-8 border-border bg-card"><ChevronRight className="h-3.5 w-3.5" /></Button>
        ...
        <div className="grid grid-cols-7">
          ...
          {days.map(d => {
            ...
            return (
              ...
              >
                <span className={cn("text-xs font-medium", isToday ? "text-[#3ecf8e] font-bold" : "text-muted-foreground")}>{d}</span>
                <div className="mt-1 space-y-0.5">
                  {dayItems.slice(0, 3).map(it => {
                    const cfg = TYPE_CONFIG[it.type] || TYPE_CONFIG.tarefa;
                    const [iconColor, bgColor] = cfg.color.split(" ");
                    const authorName = it.profiles?.full_name ? it.profiles.full_name.split(" ")[0] : null;
                    return (
                      <div
                        key={it.id}
                      ...
                      >
                        ...
                        {authorName && (
                          <span className="truncate opacity-80 text-[8px] italic flex items-center gap-0.5">
                            <Users className="h-2 w-2" /> {authorName}
                          </span>
                        )}
                      </div>
                    );
                  })}
                  {dayItems.length > 3 && <div className="text-[9px] text-muted-foreground px-1">+{dayItems.length - 3}</div>}
                </div>
              </div>
            );
          })}
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          ...
          <div className="p-4 space-y-2">
            {selectedDayItems.length === 0 ? (
              ...
              return (
                <div key={it.id} className={cn("flex items-center gap-4 p-3 bg-secondary/50 border border-border/60 rounded-lg group transition-all", isDone && "opacity-60")}>
                  ...
                  <div className="flex-1 min-w-0 cursor-pointer" onClick={() => onEdit(it)}>
                    ...
                    <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                      <span className="text-[10px] text-muted-foreground flex items-center gap-1"><Clock className="h-3 w-3" /> {timeStr}</span>
                      {it.profiles?.full_name && (
                        <span className="text-[10px] text-muted-foreground flex items-center gap-1"><Users className="h-3 w-3" /> {it.profiles.full_name}</span>
                      )}
